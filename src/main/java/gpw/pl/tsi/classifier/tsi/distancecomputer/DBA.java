...
public class DBA {
	...
	public final static double[] update(double[] C, final TimeSeries[] sequences, final int w) {
		@SuppressWarnings("unchecked")
		final ArrayList<Double>[] tupleAssociation = new ArrayList[C.length];
		for (int i = 0; i < tupleAssociation.length; i++) {
			tupleAssociation[i] = new ArrayList<Double>(sequences.length);
		...
		for (TimeSeries T : sequences) {
			seqLength = T.getValues().length;

			costMatrix[0][0] = tools.squaredEuclidean(C[0], T.getValues()[0]);
			pathMatrix[0][0] = NIL;
			optimalPathLength[0][0] = 0;
			distComputation = 1;

			for (i = 1; i < Math.min(centerLength, 1+w); i++) {
				costMatrix[i][0] = costMatrix[i - 1][0] + tools.squaredEuclidean(C[i], T.getValues()[0]);
				pathMatrix[i][0] = UP;
				optimalPathLength[i][0] = i;
			}
			if (i < centerLength)
			...
			for (j = 1; j < Math.min(seqLength, 1+w); j++) {
				costMatrix[0][j] = costMatrix[0][j - 1] + tools.squaredEuclidean(T.getValues()[j], C[0]);
				pathMatrix[0][j] = LEFT;
				optimalPathLength[0][j] = j;
			}
			if (j < seqLength)
			...
			for (i = 1; i < centerLength; i++) {
				...
				for (j = jStart; j < jStop; j++) {
					...
					costMatrix[i][j] = res + tools.squaredEuclidean(C[i], T.getValues()[j]);
					distComputation++;
				}
				if (jStop < seqLength)
					costMatrix[i][jStop] = Double.POSITIVE_INFINITY;
			...
			for (int t = nbTuplesAverageSeq - 1; t >= 0; t--) {
				tupleAssociation[i].add(T.getValues()[j]);
				switch (pathMatrix[i][j]) {
					case DIAGONAL:
						i = i - 1;
						j = j - 1;
